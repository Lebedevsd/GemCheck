...
COLORS   = os.path.join(ROOT, 'data', 'gem_colors.json')
TRANSFIG = os.path.join(ROOT, 'data', 'gem_colors_transfigured.json')
CONTENT  = os.path.join(ROOT, 'src', 'content.js')
...
def main():
    colors   = load(COLORS)
    transfig = load(TRANSFIG)

    with open(CONTENT, encoding='utf-8') as f:
        src = f.read()

    # Update GEM_COLORS
    src = replace_block(src, 'const GEM_COLORS = {', '};', fmt_colors(colors))

    # Update TRANSFIG_GEMS
    ...
    t = sum(len(v) for v in transfig.values())
    print(f'GEM_COLORS updated:   {r} base gems ({len(colors["r"])}r / {len(colors["g"])}g / {len(colors["b"])}b)')
    print(f'TRANSFIG_GEMS updated: {t} transfigured gems ({len(transfig["r"])}r / {len(transfig["g"])}g / {len(transfig["b"])}b)')
